...
pose_encode = addPositionalEncodingToSequences(pose_sequences);
%% Train DBM model
...
[dbm_mdl, dbm_train_report] = dbmTrainModel(pose_encode(~val_ind), behavior_pseudolabels(~val_ind), ... % training data
    pose_encode(val_ind), behavior_pseudolabels(val_ind), use_gpu); % validation data
...
[pseudolabel_predictions, latent_trajectories] = dbmGetModelOutputs( dbm_mdl, pose_encode, use_gpu );
